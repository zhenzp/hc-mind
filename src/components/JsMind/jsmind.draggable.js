(function ($w) {
    ...
    jsMind.draggable.prototype = {
        ...
        dragstart: function (e) {
            ...
            if(e.path[1].className=='hcmind-inner'){
                this.startX = e.clientX;
                this.startY = e.clientY;
                this.initleft = Number((e.path[1].style.left).replace('px',''))
                this.inittop = Number((e.path[1].style.top).replace('px',''))
                if(e.path[1].style.transform){
                    var arr=e.path[1].style.transform.split('(')[1].split(')')[0].split(',')
                    this.initleft = this.initleft+Number((arr[0]).replace('px',''))
                    this.inittop = this.inittop+Number((arr[1]).replace('px',''))
                }
        ...
        drag: function (e) {
            if(e.path[1].className=='hcmind-inner'&&this.dragCanvas){
                // 鼠标移动的距离
                let left = e.clientX - this.startX + this.initleft;
                let top = e.clientY - this.startY + this.inittop;
                var move=`translate(${left}px ,${top}px)`
                e.path[1].style.transform=move
                return
            }
            if (!this.jm.get_editable()) { return; }
            if (this.capture) {
        ...
        move_node: async function (src_node, target_node, target_direct) {
            if(jsMind.node.inherited(src_node, target_node)||!src_node||!target_node){
                return
            }
            var res = await this.jm.options.dragFn(src_node.id, target_node.id)
            if(!res){
                this.active_node = null;
                this.target_node = null;
                this.target_direct = null;
