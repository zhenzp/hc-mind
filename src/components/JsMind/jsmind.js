; (function ($w) {
    ...
    var DEFAULT_OPTIONS = {
        ...
        default_event_handle: {
            ...
            enable_mouseover_handle: true,
            enable_mouseout_handle: true,
        ...
        _move_node_internal: function (node, beforeid) {
            ...
            if (!!node && !!beforeid) {
                if (beforeid == '_last_') {
                    ...
                    node.index = 0;
                    this._reindex(node.parent);
                } else if(beforeid == '_up_one_'){
                    node.index=index-1.5;
                    this._reindex(node.parent);
                }  else if(beforeid == '_down_one_'){
                    node.index=index+1.5;
                    this._reindex(node.parent);
                } else{
    ...
    jm.prototype = {
        ...
        mouseover_handle: function (e) {
            if (!this.options.default_event_handle['enable_mouseover_handle']) {
                return;
            }
            ...
            if (!!nodeid) {
                if (element.tagName.toLowerCase() == 'jmnode') {
                    ...
                    let tooltipRes=this.options.tooltipFn(node);

                    if (tooltipRes.length) {
                        let oD=$c('div');
                        oD.className='tooltipCon'
                        var strShow = '';
                        tooltipRes.forEach(item => {
                            strShow+=`
                            <p>
                        ...
                        oD.innerHTML = strShow;
                        element.appendChild(oD);
                    }
                }
            } else {
                // this.select_clear();
        ...
        mouseout_handle: function (e) {
            if (!this.options.default_event_handle['enable_mouseout_handle']) {
                return;
            }
            var element = e.target || event.srcElement;
            var nodeid = this.view.get_binded_nodeid(element);
            if (!!nodeid) {
                if (element.tagName.toLowerCase() == 'jmnode') {
                    var node= this.get_node(nodeid);
                    if(element.querySelector('.tooltipCon')){
                        element.removeChild(element.querySelector('.tooltipCon'));
                    }
                }
            } else {
                // this.select_clear();
    ...
    jm.layout_provider.prototype = {
        ...
        toggle_node: async function (node) {
            if (node.isroot) {
                return;
            }
            // 异步加载下一级
            if(this.jm.options.asyncNext){
                if (node.isSync) {
                    if (node.expanded) {
                    ...
                    if (node.expanded) {
                        ...
                        var res= await this.jm.options.nextLevelFn(node)
                        console.log(`res`, res)
                        node.isSync=true;
                        this.expand_node(node);
                        var childs=res.child
                        
                        for (let i = 0; i < childs.length; i++) {
                            let el=childs[i]
                            var nodeid=el.nodeid;
                            var topic=el.topic;
                            var newNode = this.jm.add_node(node, nodeid, topic, el.data);
            ...
            console.log(`%c------toggle_node------------`, 'color:red')
            console.log(`toggle_node`, node,this)
            // if (node.expanded) {
            //     this.collapse_node(node);
            // } else {
            //     if(this.jm.options.asyncNext){
            ...
            //                 }
            //                 var newNode = this.jm.add_node(node, nodeid, topic, data);
            //                 newNode.expanded=false;
            //                 console.log(`newNode`, newNode)
            //             }
            //         }
    ...
    jm.view_provider.prototype = {
        init: function () {
            ...
            this.e_panel = $c('div');
            // 缩略图
            this.e_minimap = $c('div');
            this.e_minimap.id = 'minimap';
            // 节点元素
            ...
            // this.container.appendChild(this.e_minimap);
        },
        ...
        create_node_element: function (node, parent_node) {
            ...
            if (node.isroot) {
                ...
                var d_s = $c('jmstatus');
                $t(d_s, '');
                d_s.setAttribute('status', node.id);
                d_s.style.visibility = 'hidden';
                parent_node.appendChild(d_s);
                // view_data.hcstatus = d_s;
            }
            if (!!node.topic) {
        ...
        remove_node: function (node) {
            ...
            if (node._data.view) {
                ...
                // var hcstatus = node._data.view.hcstatus;
                this.e_nodes.removeChild(element);
                this.e_nodes.removeChild(expander);
                // this.e_nodes.removeChild(hcstatus);
                node._data.view.element = null;
                node._data.view.expander = null;
                // node._data.view.hcstatus = null;
            }
        },
        ...
        resize: function () {
            ...
            document.querySelector('.hcmind-inner')[0].style.transform='translate(0 ,0)';
            this.setZoom(1)
        ...
        setZoom: function (zoom) {
            ...
            console.log(`document.querySelector('.hcmind-inner')`, document.querySelector('.hcmind-inner'))
            let child=document.querySelector('.hcmind-inner').children;
            for (var i = 0; i < child.length; i++) {
                child[i].style.transform = 'scale(' + zoom + ')';
                child[i].style.transformOrigin= "0 0";
            };
            this.show(true);
            return true;
        ...
        show_nodes: function () {
            ...
            // var hcstatus = null;
            var _offset = this.get_view_offset();
            console.log(`show_nodes`, this)
            for (var nodeid in nodes) {
                node = nodes[nodeid];
                view_data = node._data.view;
                node_element = view_data.element;
                expander = view_data.expander;
                // hcstatus = view_data.hcstatus;
                p_expander = this.layout.get_expander_point(node);
                if (!this.layout.is_visible(node)) {
                    node_element.style.display = 'none';
                    expander.style.display = 'none';
                    // hcstatus.style.visibility = 'hidden';
                    continue;
                }
                 ...
                 */
                 node_element.className=this.jm.options.classFn(node);
                if(node.data){
                    if (node.data.hasNextNode) {
                        node.hasExpand=true
                    }else{
                ...
                if (!node.isroot && (node.children.length > 0 || node.hasExpand)) {
                    expander_text = node.data.hasExpand&&!node.isSync?'+':node.expanded ? '-' : '+';
